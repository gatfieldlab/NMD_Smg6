...
if [ "${SAMPLEDB}" == "skip" ]; then
  ...
  #done < <(grep -oE "${BASE_bis}*${raw_ext}" $SAMPLEDB)
...
#-----------------------
cd $2
logs ${SUB} "<${BASE}> Quality filtering."
pipeline_Filtering_single.sh "${BASE}" "${FILEARRAY}"
logs ${SUB} "<${BASE}> Quality filtering finished."
# if (( $? )); then
...
if (( $MAP_WITH_STAR )); then
 ...
 skipped!"
  ...
  star_input="../filtered_data/${BASE}_RiboSeq_sequentialmapping_filtered.fq"
  logs ${SUB} "<${BASE}> Found $star_input."
 ...
 genomic database using STAR..."
    ...
    trim2bp_left="/data/vricci/tools/seqtk/seqtk trimfq -b 2 ${star_input} > ${star_input%.fq}.trimmed.fq \
    2> ${__dir}/${2}/logs${SUFFIX}/${BASE}.pipeline.trimfq.log"

    star_cmd_mouse_rRNA="STAR --runThreadN ${starproc} --genomeDir=${STAR_INDEX['mouse-rrna']} \
    --readFilesIn ${star_input%.fq}.trimmed.fq --outFileNamePrefix ${star_out}STAR_1. \
    --readFilesCommand ${readFileCmd} --genomeLoad ${memory_mode} \
    --seedSearchStartLmax ${seedSearchStartLmax} --twopassMode Basic \
    ${STAR_add} \
    --outSAMtype BAM SortedByCoordinate Unsorted \
    --limitBAMsortRAM 15000000000 --outReadsUnmapped Fastx \
    --outTmpDir ${star_out}STAR_1 \
    >(tee /dev/null) -S /dev/null - 2> ${__dir}/${2}/logs${SUFFIX}/${BASE}.pipeline.STAR_1.log "

    star_cmd_human_rRNA="STAR --runThreadN ${starproc} --genomeDir=${STAR_INDEX['human-rrna']} \
    --readFilesIn ${star_out}STAR_1.Unmapped.out.mate1 --outFileNamePrefix ${star_out}STAR_2. \
    --readFilesCommand ${readFileCmd} --genomeLoad ${memory_mode} \
    --seedSearchStartLmax ${seedSearchStartLmax} --twopassMode Basic \
    ${STAR_add} \
    --outSAMtype BAM SortedByCoordinate Unsorted \
    --limitBAMsortRAM 15000000000 --outReadsUnmapped Fastx \
    --outTmpDir ${star_out}STAR_2 \
    >(tee /dev/null) -S /dev/null - 2> ${__dir}/${2}/logs${SUFFIX}/${BASE}.pipeline.STAR_2.log "

    star_cmd_mouse_tRNA="STAR --runThreadN ${starproc} --genomeDir=${STAR_INDEX['mouse-trna']} \
    --readFilesIn ${star_out}STAR_2.Unmapped.out.mate1 --outFileNamePrefix ${star_out}STAR_3. \
    --readFilesCommand ${readFileCmd} --genomeLoad ${memory_mode} \
    --seedSearchStartLmax ${seedSearchStartLmax} --twopassMode Basic \
    ${STAR_add} \
    --outSAMtype BAM SortedByCoordinate Unsorted \
    --limitBAMsortRAM 15000000000 --outReadsUnmapped Fastx \
    --outTmpDir ${star_out}STAR_3 \
    >(tee /dev/null) -S /dev/null - 2> ${__dir}/${2}/logs${SUFFIX}/${BASE}.pipeline.STAR_3.log "

    star_cmd_genome="STAR --runThreadN ${starproc} --genomeDir=${STAR_INDEX['genome']} \
    --readFilesIn ${star_out}STAR_4.Unmapped.out.mate1 --outFileNamePrefix ${star_out}STAR_4. \
    --readFilesCommand ${readFileCmd} --genomeLoad ${memory_mode} \
    ...
    --limitBAMsortRAM 15000000000 --outReadsUnmapped Fastx \
    --outTmpDir ${star_out}STAR_4 \
    >(tee /dev/null) -S /dev/null - 2> ${__dir}/${2}/logs${SUFFIX}/${BASE}.pipeline.STAR_4.log "

    star_cmd="${trim2bp_left}; \
      ${star_cmd_mouse_rRNA}; \
	    ${star_cmd_human_rRNA}; \
	    ${star_cmd_mouse_tRNA}; \
	    $star_cmd_genome"
    star_rename="mv ${star_out}STAR_4.Aligned.out.bam ${star_out}Aligned.out.bam; \
    mv ${star_out}STAR_4.Aligned.sortedByCoord.out.bam ${star_out}Aligned.sortedByCoord.out.bam; \
    mv ${star_out}STAR_4.Log.out ${star_out}Log.out; \
    mv ${star_out}STAR_4.Log.final.out ${star_out}Log.final.out; \
    mv ${star_out}STAR_4.Log.progress.out ${star_out}Log.progress.out; \
    mv ${star_out}STAR_4.SJ.out.tab ${star_out}SJ.out.tab; \
    mv ${star_out}STAR_4._STARgenome ${star_out}_STARgenome; \
    mv ${star_out}STAR_4._STARpass1 ${star_out}_STARpass1; \
    mv ${star_out}STAR_4.Unmapped.out.mate1 ${star_out}Unmapped.out.mate1"
...
EOF
    ...
    current_dir=$(pwd)
    logs ${SUB} "<${BASE}> Current directory: $current_dir"
    eval ${star_cmd}
    eval ${star_rename}
    cur_err=$?
    if (( $cur_err )); then
